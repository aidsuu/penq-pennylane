...
def main():
    print("=== 3D Cubic TFIM Demo (Mapped-Lattice Workflow) ===")
    ...
    comparison = compare_cubic_tfim_exact_vs_mps(
        Lx=2,
        Ly=2,
        Lz=2,
        Jx=1.0,
    ...
    print(
        ...
        f"zz_x={exact['nn_zz_x']:.8f} zz_y={exact['nn_zz_y']:.8f} zz_z={exact['nn_zz_z']:.8f}"
    )

    print("\nMPS mapped approximation (3x2x2) observables:")
    print(f"energy={mps['energy']:.8f} energy_per_site={mps['energy_per_site']:.8f}")
    print(
        f"mx={mps['magnetization_x']:.8f} mz={mps['magnetization_z']:.8f} "
        f"zz_x={mps['nn_zz_x']:.8f} zz_y={mps['nn_zz_y']:.8f} zz_z={mps['nn_zz_z']:.8f}"
    )

    print("\nExact-vs-MPS comparison (2x2x2):")
    print(
