...
from QML.PenQ.examples.square_tfim_dynamics_utils import read_square_tfim_imag_time_csv
from QML.PenQ.examples.square_tfim_dynamics_utils import read_square_tfim_real_time_csv
...
def run_square_tfim_dynamics_report(real_csv="square_tfim_real_time.csv", imag_csv="square_tfim_imag_time.csv", output_dir="."):
    ...
    if real_path.exists():
        real_rows = read_square_tfim_real_time_csv(real_path)
    else:
        real_rows = square_tfim_real_time_rows()
        write_square_tfim_real_time_csv(real_path, real_rows)

    imag_path = pathlib.Path(imag_csv)
    if imag_path.exists():
        imag_rows = read_square_tfim_imag_time_csv(imag_path)
    else:
        imag_rows = square_tfim_imag_time_rows()
        write_square_tfim_imag_time_csv(imag_path, imag_rows)
    ...
    finalize_axes(ax_rt_energy, "Time", "Energy", "Square TFIM Real-Time Energy vs Time")
    ax_rt_energy.legend(frameon=False)
    save_required_figure_outputs(fig_rt_energy, output_dir, "square_tfim_real_time_energy_vs_time")
    ...
    finalize_axes(ax_rt_obs, "Time", "Observable", "Square TFIM Real-Time Observables vs Time")
    ax_rt_obs.legend(frameon=False, ncol=2)
    save_required_figure_outputs(fig_rt_obs, output_dir, "square_tfim_real_time_observables_vs_time")
    ...
    finalize_axes(ax_it_energy, "Imag-Time Step", "Energy", "Square TFIM Imag-Time Energy vs Step")
    ax_it_energy.legend(frameon=False)
    save_required_figure_outputs(fig_it_energy, output_dir, "square_tfim_imag_time_energy_vs_step")
    ...
    finalize_axes(ax_it_cmp, "Exact Energy", "MPS Energy", "Square TFIM Imag-Time Exact vs MPS")
    save_required_figure_outputs(fig_it_cmp, output_dir, "square_tfim_imag_time_exact_vs_mps")
...
def main():
    parser = argparse.ArgumentParser(description="Scientific plots for square TFIM real-time and imag-time trajectories.")
    parser.add_argument("--real-csv", default="square_tfim_real_time.csv", help="Input or generated real-time CSV path.")
    parser.add_argument("--imag-csv", default="square_tfim_imag_time.csv", help="Input or generated imaginary-time CSV path.")
    parser.add_argument("--output-dir", default=".", help="Output directory for plots.")
    args = parser.parse_args()

    report = run_square_tfim_dynamics_report(real_csv=args.real_csv, imag_csv=args.imag_csv, output_dir=args.output_dir)
    print("workflow=2D square TFIM dynamics report")
    print(f"style={report['style_used']}")
    print("saved=square_tfim_real_time_energy_vs_time.[png|pdf]")
    print("saved=square_tfim_real_time_observables_vs_time.[png|pdf]")
    print("saved=square_tfim_imag_time_energy_vs_step.[png|pdf]")
    print("saved=square_tfim_imag_time_exact_vs_mps.[png|pdf]")
