...
from QML.PenQ import cubic_tfim_observables


CUBIC_TFIM_SCAN_FIELDNAMES = [
    "Lx",
    "Ly",
    "Lz",
    "n_sites",
...
def cubic_tfim_scan_rows(
    lattice_sizes=((2, 2, 2), (3, 2, 2)),
    Jx_values=(1.0,),
    Jy_values=(1.0,),
    Jz_values=(1.0,),
                            ...
                            rows.append(
                                {
                                    ...
                                    "nn_zz_x": float(exact["nn_zz_x"]),
                                    "nn_zz_y": float(exact["nn_zz_y"]),
                                    "nn_zz_z": float(exact["nn_zz_z"]),
                                    "max_bond_dim": "",
                            ...
                            rows.append(
                                {
                                    ...
                                    "nn_zz_x": float(mps["nn_zz_x"]),
                                    "nn_zz_y": float(mps["nn_zz_y"]),
                                    "nn_zz_z": float(mps["nn_zz_z"]),
...
def write_cubic_tfim_scan_csv(path, rows):
    path = pathlib.Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=CUBIC_TFIM_SCAN_FIELDNAMES)
        writer.writeheader()
        writer.writerows(rows)


def main():
    parser = argparse.ArgumentParser(description="Deterministic 3D cubic-lattice TFIM scan.")
    parser.add_argument("--csv", dest="csv_path", default="cubic_tfim_scan.csv", help="Scan CSV output path.")
    ...
    parser.add_argument("--Jz-grid", dest="Jz_grid", default="1.0", help="Comma-separated Jz grid.")
    parser.add_argument(
        "--mps-bonds",
        dest="mps_bonds",
        default="4,8",
    ...
    rows = cubic_tfim_scan_rows(
        ...
        Jz_values=parse_float_grid(args.Jz_grid),
        h_values=parse_float_grid(args.h_grid),
        mps_bond_dims=parse_int_grid(args.mps_bonds),
    )
    write_cubic_tfim_scan_csv(args.csv_path, rows)

    print("workflow=3D cubic TFIM scan")
    print("mapping=s(x,y,z)=x+Lx*y+Lx*Ly*z")
    print("hamiltonian=H_3D=-Jx sum_<i,j>_x ZZ - Jy sum_<i,j>_y ZZ - Jz sum_<i,j>_z ZZ - h sum_i X")
